...
object AuthBuilder {
  ...
  def withAuthorisedUser(
    ...
    when(
      mockAuthConnector.authorise(any, retrieval = ArgumentMatchers.eq(allEnrolments))(
        any[HeaderCarrier],
    ...
    when(
      mockAuthConnector
        .authorise(any, retrieval = ArgumentMatchers.eq(email and credentialRole and affinityGroup))(
          any[HeaderCarrier],
          any[ExecutionContext]
        )
    ...
    when(
      mockAuthConnector.authorise(any, retrieval = ArgumentMatchers.eq(credentials))(
        any[HeaderCarrier],
        any[ExecutionContext]
      )
    )
    ...
    when(
      mockAuthConnector.authorise(any, retrieval = ArgumentMatchers.eq(affinityGroup))(
        any[HeaderCarrier],
        any[ExecutionContext]
      )
    )
  ...
  def withNotLoggedInUser(mockAuthConnector: AuthConnector): Unit = {
    ...
    when(
      mockAuthConnector.authorise(any, any[Retrieval[_]])(
